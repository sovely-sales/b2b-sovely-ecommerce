    ...
    try {
        ...
        if (fs.existsSync(logoPath)) {
            try {
                ...
                doc.fillColor('#1e293b').fontSize(22).font('Helvetica-Bold').text('INFINITY ENTERPRISES', 40, 30);
            }
        } else {
            doc.fillColor('#1e293b').fontSize(22).font('Helvetica-Bold').text('INFINITY ENTERPRISES', 40, 30);
        }
...
export const generateInvoiceBuffer = async (invoice, user) => {
    return new Promise((resolve, reject) => {
        try {
            ...
            doc.fontSize(24).font('Helvetica-Bold').fillColor('#0f172a').text('INFINITY ENTERPRISES', 40, 35);
            doc.fillColor('#0f172a')
                .fontSize(18)
                .font('Helvetica-Bold')
